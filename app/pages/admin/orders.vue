  ...
  <div class="page">
    <main class="wrap">
      <section class="card">
        ...
        <!-- ✅ фильтры (чтобы не было ошибок filter/filtered) -->
        <div class="filters">
        ...
        <div v-else class="list">
          <div v-for="o in filtered" :key="o.id" class="item">
            <div class="row">
              <div class="left">
                ...
                <div class="meta">
                  ...
                  <span v-if="o.paymentMethod === 'cash' && o.changeFrom">
                    • Сдача с: <b>{{ o.changeFrom }} MDL</b>
                  </span>

                  <span v-if="o.paymentMethod === 'cash' && o.changeDue != null">
                    • Сдача дать: <b>{{ o.changeDue }} MDL</b>
                  </span>
                </div>
            ...
            <div class="actions">
              <button class="btn btn--primary" type="button" @click="markPaid(o.id)" :disabled="o.paid">
                Отметить “Оплачен”
              </button>
...
/** ✅ проверка доступа (чтобы не ловить NavigationGuard типы) */
onMounted(async () => {
  try {
...
/** ✅ useFetch без method DELETE — только GET */
...
/** ✅ обёртка, чтобы убрать ошибку onClick */
...
async function markPaid(id: string) {
  await $fetch(`/api/admin/orders/${encodeURIComponent(id)}/paid`, { method: "PATCH" })
  showToast("Оплачено ✅")
  refreshOrders()
}
...
/** ✅ теперь copyReceipt точно существует (и ошибка пропадёт) */
function copyReceipt(o: ApiOrder) {
  const lines: string[] = []
  lines.push(`For Love Coffee — Чек`)
  lines.push(`Заказ: ${o.id}`)
  lines.push(`Дата: ${formatDate(o.createdAt)}`)
  lines.push(`Готово через: ${o.prepMinutes} мин`)
  lines.push(`Готово к: ${formatTime(o.readyAt)}`)
  lines.push(`Клиент: ${o.customerName} (${o.customerPhone})`)

  lines.push(`Способ: ${o.deliveryType === "delivery" ? "Доставка" : "Самовывоз"}`)
  if (o.deliveryAddress) lines.push(`Адрес: ${o.deliveryAddress}`)
  if (o.deliveryType === "delivery") {
    lines.push(`Доставка: ${o.deliveryDistanceKm} км × 10 = ${o.deliveryFee} MDL`)
  }

  lines.push(`Оплата: ${o.paymentMethod === "cash" ? "Наличными" : "Картой"} при получении`)
  if (o.paymentMethod === "cash" && o.changeFrom) lines.push(`Сдача с: ${o.changeFrom} MDL`)
  if (o.paymentMethod === "cash" && o.changeDue != null) lines.push(`Сдача дать: ${o.changeDue} MDL`)
  ...
  if (o.deliveryType === "delivery") lines.push(`Доставка: ${o.deliveryFee} MDL`)
