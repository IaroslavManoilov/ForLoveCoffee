...
export default defineEventHandler(async (event) => {
  ...
  if (!Number.isFinite(prepMinutes) || prepMinutes <= 0) throw createError({ statusCode: 400, statusMessage: "Bad prepMinutes" })

  // подсчёты на сервере (чтобы никто не подменил цены)
  ...
  await run(
    `INSERT INTO orders (
      ...
      prepMinutes, readyAt,
      paymentMethod, paid, changeFrom, changeDue,
      comment,
      subtotal, total
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
  ...
  for (const i of normalized) {
    await run(
      `INSERT INTO order_items (orderId, id, title, price, qty, category) VALUES (?, ?, ?, ?, ?, ?)`,
